...
// — someone who has debugged this at 3am more times than they'd like to admit
...
// shared by execfind, execdelete, execwrite, execread. stop copy-pasting lambdas.
// deja vu intensifies every time i see another as_i64 lambda somewhere
// ═════════════════════════════════════════

int64_t HyperDBManager::CoerceI64(const HyperValue& v) noexcept {
    return std::visit([](auto&& x) -> int64_t {
        using T = std::decay_t<decltype(x)>;
        if constexpr (std::is_arithmetic_v<T>) return static_cast<int64_t>(x);
        return 0;
    }, v);
}
...
uint64_t HyperDBManager::CoerceU64(const HyperValue& v) noexcept {
    return std::visit([](auto&& x) -> uint64_t {
        using T = std::decay_t<decltype(x)>;
        if constexpr (std::is_arithmetic_v<T>) return static_cast<uint64_t>(x);
        return 0;
    }, v);
}
...
void HyperDBQueue::AddToQueueBulk(std::vector<QueueEntry> entries) {
    ...
    {
        std::lock_guard<std::mutex> lock(mutex_);
        for (auto& e : entries) queue_.push(std::move(e));
    }
    queue_size_ += count;
    cv_.notify_one();
}

bool HyperDBQueue::IsEmpty() {
    return queue_size_ == 0 && !worker_busy_;
}
    ...
    while (!stop_worker_) {
...

        {
            ...
            if (!queue_.empty()) {
                ...
                has_entry = true;
                auto t = entry.action;
                is_callback_action = (t == QueueActionType::Read || t == QueueActionType::Find);
            }
        }
...
void HyperDBQueue::Execute(QueueEntry entry) {
    switch (entry.action) {
    case QueueActionType::CreateDatabase: manager_->ExecCreateDatabase(entry.db_name);                                               break;
    case QueueActionType::CreateTable:    manager_->ExecCreateTable(entry.table_name, entry.column_defs);                            break;
    case QueueActionType::DropTable:      manager_->ExecDropTable(entry.table_name);                                                 break;
...
void HyperDBManager::OpenDB(const std::string& path, const std::string& password) {
    ...
    if (!file.is_open()) throw std::runtime_error("HyperDB: failed to open " + path);
...
void HyperDBManager::QueueCreateDatabase(const std::string& name) {
    queue_.AddToQueue(QueueEntry{ .action = QueueActionType::CreateDatabase, .db_name = name });
}
void HyperDBManager::QueueCreateTable(const std::string& table_name, std::vector<ColumnDef> cols) {
    queue_.AddToQueue(QueueEntry{ .action = QueueActionType::CreateTable, .table_name = table_name, .column_defs = std::move(cols) });
}
void HyperDBManager::QueueDropTable(const std::string& table_name) {
    queue_.AddToQueue(QueueEntry{ .action = QueueActionType::DropTable, .table_name = table_name });
}
void HyperDBManager::QueueClearTable(const std::string& table_name) {
    queue_.AddToQueue(QueueEntry{ .action = QueueActionType::ClearTable, .table_name = table_name });
}
void HyperDBManager::QueueClearColumn(const std::string& table_name, const std::string& column_name) {
    queue_.AddToQueue(QueueEntry{ .action = QueueActionType::ClearColumn, .table_name = table_name, .column_name = column_name });
}
void HyperDBManager::QueueWrite(const std::string& table_name, std::vector<RowData> row) {
    QueueEntry e;
...
void HyperDBManager::QueueWriteBulk(const std::string& table_name, std::vector<std::vector<RowData>> rows) {
...
void HyperDBManager::QueueRead(const std::string& table_name, uint64_t row_id, std::function<void(ReadResult)> callback) {
    queue_.AddToQueue(QueueEntry{ .action = QueueActionType::Read, .table_name = table_name, .row_id = row_id, .callback = std::move(callback) });
}
...
size_t HyperDBManager::EstimateMirrorSize() {
    ...
    for (auto& table : mirror_.tables)
        for (auto& col : table.columns) {
            ...
            for (auto& s : col.strings) total += s.size();
        }
    return total;
}
...
void HyperDBManager::ExecCreateTable(const std::string& table_name, const std::vector<ColumnDef>& cols) {
    std::lock_guard<std::mutex> lock(data_mutex_);
    for (auto& t : mirror_.tables) if (t.name == table_name) return; // already exists, touch nothing
...
void HyperDBManager::ExecClearColumn(const std::string& table_name, const std::string& column_name) {
    std::lock_guard<std::mutex> lock(data_mutex_);
    TableMirror*  table = FindTable(table_name);
    if (!table) return;
    ...
    col->f32.clear(); col->f64.clear();   col->bools.clear();  col->strings.clear();
    dirty_ = true;
}

void HyperDBManager::ExecWrite(const std::string& table_name, const std::vector<RowData>& row) {
    std::lock_guard<std::mutex> lock(data_mutex_);
    TableMirror* table = FindTable(table_name);
    if (!table) return;
    ...
    for (auto& rd : row) {
        ...
        if (it == table->column_map.end()) continue;
...
void HyperDBManager::ExecRead(const std::string& table_name, uint64_t row_id, const std::function<void(ReadResult)>& callback) {
    ReadResult result;
    {
        std::lock_guard<std::mutex> lock(data_mutex_);
        TableMirror* table = FindTable(table_name);
...
void HyperDBManager::ExecFind(const std::string& table_name, const std::string& col_name,
    ...
    std::vector<ReadResult> results;
    {
        ...
        if (!col)   { if (callback) callback({}); return; }
        ...
        const std::string needle_str = (col->type == HyperDB::ColumnType::ColumnType_String)
            ? HyperDBUtil::HyperValueToString(val) : std::string{};
        ...
        switch (col->type) {
        case HyperDB::ColumnType::ColumnType_Int8:   { auto n = static_cast<int8_t>(needle_i64);   for (uint64_t i = 0; i < table->row_count; ++i) if (col->i8[i]   == n) hits.push_back(i); break; }
        case HyperDB::ColumnType::ColumnType_Int16:  { auto n = static_cast<int16_t>(needle_i64);  for (uint64_t i = 0; i < table->row_count; ++i) if (col->i16[i]  == n) hits.push_back(i); break; }
        case HyperDB::ColumnType::ColumnType_Int32:  { auto n = static_cast<int32_t>(needle_i64);  for (uint64_t i = 0; i < table->row_count; ++i) if (col->i32[i]  == n) hits.push_back(i); break; }
        case HyperDB::ColumnType::ColumnType_Int64:  {                                              for (uint64_t i = 0; i < table->row_count; ++i) if (col->i64[i]  == needle_i64) hits.push_back(i); break; }
        case HyperDB::ColumnType::ColumnType_UInt8:  { auto n = static_cast<uint8_t>(needle_u64);  for (uint64_t i = 0; i < table->row_count; ++i) if (col->u8[i]   == n) hits.push_back(i); break; }
        case HyperDB::ColumnType::ColumnType_UInt16: { auto n = static_cast<uint16_t>(needle_u64); for (uint64_t i = 0; i < table->row_count; ++i) if (col->u16[i]  == n) hits.push_back(i); break; }
        case HyperDB::ColumnType::ColumnType_UInt32: { auto n = static_cast<uint32_t>(needle_u64); for (uint64_t i = 0; i < table->row_count; ++i) if (col->u32[i]  == n) hits.push_back(i); break; }
        case HyperDB::ColumnType::ColumnType_UInt64: {                                              for (uint64_t i = 0; i < table->row_count; ++i) if (col->u64[i]  == needle_u64) hits.push_back(i); break; }
        case HyperDB::ColumnType::ColumnType_Float32: {
            // float equality. i am not sorry. if you want epsilon control, make a rangeFind.
            auto n = static_cast<float>(needle_f64);
            for (uint64_t i = 0; i < table->row_count; ++i)
                if (std::fabsf(col->f32[i] - n) < 1e-6f) hits.push_back(i);
            break;
        }
        case HyperDB::ColumnType::ColumnType_Float64: {
            for (uint64_t i = 0; i < table->row_count; ++i)
                if (std::fabs(col->f64[i] - needle_f64) < 1e-6) hits.push_back(i);
            break;
        }
        case HyperDB::ColumnType::ColumnType_Bool: {
            bool n = (needle_i64 != 0);
            for (uint64_t i = 0; i < table->row_count; ++i)
                if ((col->bools[i] != 0) == n) hits.push_back(i);
            break;
        }
        case HyperDB::ColumnType::ColumnType_String: {
            for (uint64_t i = 0; i < table->row_count; ++i)
                if (col->strings[i] == needle_str) hits.push_back(i);
            break;
        }
        default: break;
        }
        ...
        // GetValueAtIndex boxes, but we only call it on actual hits, not every row. much better.
...
void HyperDBManager::ExecDelete(const std::string& table_name, const std::string& col_name,
    ...
    switch (col->type) {
    case HyperDB::ColumnType::ColumnType_Int8:   { auto n = static_cast<int8_t>(needle_i64);   for (uint64_t i = 0; i < table->row_count; ++i) if (col->i8[i]   == n) to_delete.push_back(i); break; }
    case HyperDB::ColumnType::ColumnType_Int16:  { auto n = static_cast<int16_t>(needle_i64);  for (uint64_t i = 0; i < table->row_count; ++i) if (col->i16[i]  == n) to_delete.push_back(i); break; }
    case HyperDB::ColumnType::ColumnType_Int32:  { auto n = static_cast<int32_t>(needle_i64);  for (uint64_t i = 0; i < table->row_count; ++i) if (col->i32[i]  == n) to_delete.push_back(i); break; }
    case HyperDB::ColumnType::ColumnType_Int64:  {                                              for (uint64_t i = 0; i < table->row_count; ++i) if (col->i64[i]  == needle_i64) to_delete.push_back(i); break; }
    case HyperDB::ColumnType::ColumnType_UInt8:  { auto n = static_cast<uint8_t>(needle_u64);  for (uint64_t i = 0; i < table->row_count; ++i) if (col->u8[i]   == n) to_delete.push_back(i); break; }
    case HyperDB::ColumnType::ColumnType_UInt16: { auto n = static_cast<uint16_t>(needle_u64); for (uint64_t i = 0; i < table->row_count; ++i) if (col->u16[i]  == n) to_delete.push_back(i); break; }
    case HyperDB::ColumnType::ColumnType_UInt32: { auto n = static_cast<uint32_t>(needle_u64); for (uint64_t i = 0; i < table->row_count; ++i) if (col->u32[i]  == n) to_delete.push_back(i); break; }
    case HyperDB::ColumnType::ColumnType_UInt64: {                                              for (uint64_t i = 0; i < table->row_count; ++i) if (col->u64[i]  == needle_u64) to_delete.push_back(i); break; }
    case HyperDB::ColumnType::ColumnType_Float32: {
        auto n = static_cast<float>(needle_f64);
        for (uint64_t i = 0; i < table->row_count; ++i)
            if (std::fabsf(col->f32[i] - n) < 1e-6f) to_delete.push_back(i);
        break;
    }
    case HyperDB::ColumnType::ColumnType_Float64: {
        for (uint64_t i = 0; i < table->row_count; ++i)
            if (std::fabs(col->f64[i] - needle_f64) < 1e-6) to_delete.push_back(i);
        break;
    }
    case HyperDB::ColumnType::ColumnType_Bool: {
        bool n = (needle_i64 != 0);
        for (uint64_t i = 0; i < table->row_count; ++i)
            if ((col->bools[i] != 0) == n) to_delete.push_back(i);
        break;
    }
    case HyperDB::ColumnType::ColumnType_String: {
        for (uint64_t i = 0; i < table->row_count; ++i)
            if (col->strings[i] == needle_str) to_delete.push_back(i);
        break;
    }
    default: break;
    }
...
ColumnMirror* HyperDBManager::FindColumn(TableMirror& table, const std::string& name) {
    auto it = table.column_map.find(name);
    if (it != table.column_map.end()) return &table.columns[it->second];
    return nullptr;
}

HyperValue HyperDBManager::GetValueAtIndex(const ColumnMirror& col, uint64_t idx) {
    ...
    switch (col.type) {
    case HyperDB::ColumnType::ColumnType_Int8:    return col.i8.at(idx);
    case HyperDB::ColumnType::ColumnType_Int16:   return col.i16.at(idx);
    case HyperDB::ColumnType::ColumnType_Int32:   return col.i32.at(idx);
    case HyperDB::ColumnType::ColumnType_Int64:   return col.i64.at(idx);
    case HyperDB::ColumnType::ColumnType_UInt8:   return col.u8.at(idx);
    case HyperDB::ColumnType::ColumnType_UInt16:  return col.u16.at(idx);
    case HyperDB::ColumnType::ColumnType_UInt32:  return col.u32.at(idx);
    case HyperDB::ColumnType::ColumnType_UInt64:  return col.u64.at(idx);
    case HyperDB::ColumnType::ColumnType_Float32: return col.f32.at(idx);
    case HyperDB::ColumnType::ColumnType_Float64: return col.f64.at(idx);
    case HyperDB::ColumnType::ColumnType_Bool:    return static_cast<bool>(col.bools.at(idx));
    case HyperDB::ColumnType::ColumnType_String:  return col.strings.at(idx);
    default: return int8_t(0); // if we reach here, the schema changed and i have no idea what's going on
    }
}
...
flatbuffers::Offset<HyperDB::Database> HyperDBManager::SerializeFromMirror(flatbuffers::FlatBufferBuilder& builder) {
    ...
    for (auto& tm : mirror_.tables) {
        ...
        for (auto& cm : tm.columns) {
            ...
            switch (cm.type) {
            case HyperDB::ColumnType::ColumnType_Int8:    { auto t = HyperDB::CreateInt8Column(builder,    builder.CreateVector(cm.i8));                 col_offset = HyperDB::CreateColumn(builder, name_off, cm.type, HyperDB::ColumnData::ColumnData_Int8Column,    t.Union()); break; }
            case HyperDB::ColumnType::ColumnType_Int16:   { auto t = HyperDB::CreateInt16Column(builder,   builder.CreateVector(cm.i16));                col_offset = HyperDB::CreateColumn(builder, name_off, cm.type, HyperDB::ColumnData::ColumnData_Int16Column,   t.Union()); break; }
            case HyperDB::ColumnType::ColumnType_Int32:   { auto t = HyperDB::CreateInt32Column(builder,   builder.CreateVector(cm.i32));                col_offset = HyperDB::CreateColumn(builder, name_off, cm.type, HyperDB::ColumnData::ColumnData_Int32Column,   t.Union()); break; }
            case HyperDB::ColumnType::ColumnType_Int64:   { auto t = HyperDB::CreateInt64Column(builder,   builder.CreateVector(cm.i64));                col_offset = HyperDB::CreateColumn(builder, name_off, cm.type, HyperDB::ColumnData::ColumnData_Int64Column,   t.Union()); break; }
            case HyperDB::ColumnType::ColumnType_UInt8:   { auto t = HyperDB::CreateUInt8Column(builder,   builder.CreateVector(cm.u8));                 col_offset = HyperDB::CreateColumn(builder, name_off, cm.type, HyperDB::ColumnData::ColumnData_UInt8Column,   t.Union()); break; }
            case HyperDB::ColumnType::ColumnType_UInt16:  { auto t = HyperDB::CreateUInt16Column(builder,  builder.CreateVector(cm.u16));                col_offset = HyperDB::CreateColumn(builder, name_off, cm.type, HyperDB::ColumnData::ColumnData_UInt16Column,  t.Union()); break; }
            case HyperDB::ColumnType::ColumnType_UInt32:  { auto t = HyperDB::CreateUInt32Column(builder,  builder.CreateVector(cm.u32));                col_offset = HyperDB::CreateColumn(builder, name_off, cm.type, HyperDB::ColumnData::ColumnData_UInt32Column,  t.Union()); break; }
            case HyperDB::ColumnType::ColumnType_UInt64:  { auto t = HyperDB::CreateUInt64Column(builder,  builder.CreateVector(cm.u64));                col_offset = HyperDB::CreateColumn(builder, name_off, cm.type, HyperDB::ColumnData::ColumnData_UInt64Column,  t.Union()); break; }
            case HyperDB::ColumnType::ColumnType_Float32: { auto t = HyperDB::CreateFloat32Column(builder, builder.CreateVector(cm.f32));               col_offset = HyperDB::CreateColumn(builder, name_off, cm.type, HyperDB::ColumnData::ColumnData_Float32Column, t.Union()); break; }
            case HyperDB::ColumnType::ColumnType_Float64: { auto t = HyperDB::CreateFloat64Column(builder, builder.CreateVector(cm.f64));               col_offset = HyperDB::CreateColumn(builder, name_off, cm.type, HyperDB::ColumnData::ColumnData_Float64Column, t.Union()); break; }
            case HyperDB::ColumnType::ColumnType_Bool:    { auto t = HyperDB::CreateBoolColumn(builder,    builder.CreateVector(cm.bools));              col_offset = HyperDB::CreateColumn(builder, name_off, cm.type, HyperDB::ColumnData::ColumnData_BoolColumn,    t.Union()); break; }
            case HyperDB::ColumnType::ColumnType_String:  { auto t = HyperDB::CreateStringColumn(builder,  builder.CreateVectorOfStrings(cm.strings));  col_offset = HyperDB::CreateColumn(builder, name_off, cm.type, HyperDB::ColumnData::ColumnData_StringColumn,  t.Union()); break; }
            default: break;
            }
        ...
        auto tbl = HyperDB::CreateTable(builder,
            builder.CreateString(tm.name),
            builder.CreateVector(col_offsets),
            tm.row_count);
        table_offsets.push_back(tbl);
    }

    return HyperDB::CreateDatabase(builder,
        builder.CreateString(mirror_.name),
...
void HyperDBCluster::Flush(uint32_t iterations) {
    for (auto& shard : shards_) shard->FlushDB(iterations);
    SaveManifest();
}

void HyperDBCluster::SetFlushInterval(int64_t ms) {
    for (auto& shard : shards_) shard->SetFlushInterval(ms);
}
...
bool HyperDBCluster::IsQueueEmpty() {
    for (auto& shard : shards_) if (!shard->IsQueueEmpty()) return false;
    return true;
}

void HyperDBCluster::QueueCreateTable(const std::string& table_name, std::vector<ColumnDef> cols) {
...
void HyperDBCluster::QueueDropTable(const std::string& table_name, ShardTarget target) {
    ForEachShard(target, [&](HyperDBManager& shard) { shard.QueueDropTable(table_name); });
    if (target == ShardTarget::All) manifest_tables_.erase(table_name);
}

void HyperDBCluster::QueueClearTable(const std::string& table_name, ShardTarget target) {
    ForEachShard(target, [&](HyperDBManager& shard) { shard.QueueClearTable(table_name); });
    if (target == ShardTarget::All) manifest_tables_[table_name].clear();
}

void HyperDBCluster::QueueClearColumn(const std::string& table_name, const std::string& column_name, ShardTarget target) {
    ForEachShard(target, [&](HyperDBManager& shard) { shard.QueueClearColumn(table_name, column_name); });
}

void HyperDBCluster::QueueWrite(const std::string& table_name, std::vector<RowData> row) {
...
void HyperDBCluster::QueueFind(const std::string& table_name, const std::string& column_name,
                                HyperValue value,
    ...
    // shared_ptr soup because lambdas can't capture by ref safely across async ops.
    // i am not sure why this works but it fixes the problem.
    auto shared_results   = std::make_shared<std::vector<ReadResult>>();
    auto shared_remaining = std::make_shared<std::atomic<int>>(static_cast<int>(indices.size()));
    auto shared_mutex     = std::make_shared<std::mutex>();

    for (int idx : indices) {
        shards_[idx]->QueueFind(table_name, column_name, value,
            [shared_results, shared_remaining, shared_mutex, callback]
            (std::vector<ReadResult> partial) {
                {
                    std::lock_guard<std::mutex> lock(*shared_mutex);
                    for (auto& r : partial) shared_results->push_back(std::move(r));
                }
                if (--(*shared_remaining) == 0)
                    if (callback) callback(std::move(*shared_results));
            });
    }
}
...
void HyperDBCluster::MaybeSpillToNewShard() {
    if (ActiveShard().EstimateMirrorSize() < shard_limit_) return;
...
std::pair<int, uint64_t> HyperDBCluster::ResolveRowId(const std::string& table_name, uint64_t global_row_id) {
    auto it = manifest_tables_.find(table_name);
    if (it == manifest_tables_.end()) return { -1, 0 };
...
std::vector<int> HyperDBCluster::GetShardIndices(ShardTarget target) {
    ...
    for (int i = 0; i < static_cast<int>(shards_.size()); i++) {
        if (target == ShardTarget::All)                                       indices.push_back(i);
        else if (target == ShardTarget::ActiveOnly && i == active_shard_index_) indices.push_back(i);
        else if (target == ShardTarget::OldOnly    && i != active_shard_index_) indices.push_back(i);
    }
    return indices;
}

void HyperDBCluster::ForEachShard(ShardTarget target, std::function<void(HyperDBManager&)> fn) {
    for (int idx : GetShardIndices(target)) fn(*shards_[idx]);
}
...
void HyperDBCluster::LoadManifest() {
    ...
    if (!f.is_open()) throw std::runtime_error("HyperDBCluster: failed to open manifest " + manifest_path_);
